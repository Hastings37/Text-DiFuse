...
'''
高频细节增强损失函数，为了保证模型在mask区域上生成更强的边缘和纹理内容，保证融合，生成图像可以更加清晰，锐利，可以中带你突出想要的目标区域内容； 
'''
def high_freq_enhancement_loss(x0t_grad: torch.Tensor,
    ...
    loss_per_batch = - sum_grad / count_mask       # B 取负的平均梯度内容；
...
def local_contrast_enhancement_loss(x: torch.Tensor,
    ...
    # 局部标准差，大：区域的边缘和纹理明显 小：区域平滑，低对比度；

    masked_std = local_std * mask
    sum_std   = masked_std.sum(dim=(1,2,3))                               
    count     = mask.sum(dim=(1,2,3)).clamp(min=eps)  # 防止除以0的情况出现；

    loss_per_sample = - sum_std / count                                      
    loss = loss_per_sample.mean()
    # 最小化这个数值，就是最大化 sum_std/count
    # 增大局部的标准差，提高局部的对比度；

    return loss

'''从一个文件夹中获取到全部的图像路径，其中可以包含有子目录'''
def get_Images(dir):
    images = []
    assert os.path.isdir(dir), f'{dir} is not a valid directory'
    
    for root, _, fnames in sorted(os.walk(dir)):
        # root dirs files 下面的子目录的名称+下面的所有文件名 没有路径；
        for fname in sorted(fnames):
            if is_image_file(fname):
                path = os.path.join(root, fname) # 路径+文件名称
                images.append(path)
    return images


# 不行啊后面要使用文本内容还是需要自己写这里的内容的；
...
'''
两个图片  ir vis Y_ir Y_vis Cr Cb
         vis vis Y_vis Y_vis Cr Cb
         vis ir Y_ir Y_ir Cr Cb 保证得到的CbCr是完整的色彩信息；
'''
def train_transform(img1: Image.Image,
    ...
    def to_tensor_YCB(im):
        arr = np.array(im.convert('YCbCr')).astype(np.float32) / 127.5 - 1.0 # -1 to 1
        arr = np.transpose(arr, (2, 0, 1))
        c, hh, ww = arr.shape
        arr = arr[:, :hh - hh % 16, :ww - ww % 16] # make divisible by 16 转换为16的整数倍数
        return torch.from_numpy(arr) # 转换为Ycbcr的tensor内容
...
'''
图像的尺度，范围，最小边不超过MAX_SIZE，且是16的整数倍
最小的边不超过MAX_SIZE 并且转换为从 -1 到 1 的 BCHW 的tensor形式； 
'''
def test_transform(img,MAX_SIZE):
    ...
    img = np.array(img)
    img = img.astype(np.float32) / 127.5 - 1  # -1 to 1
...
"""
将图像的tensor转换为numpy的图像格式 范围从-1 1 转换到255的numpy 
后面可以用来直接保存图像；
"""
def to_numpy_image(images):
    # .clone().detach().cpu().numpy()
    images = copy.copy(images)
    images = images.detach().cpu().numpy() + 1 # 0 to 2
...
class GET_TestDataset(Dataset):
    def __init__(self, SourceImage1_Path, SourceImage2_Path, MAX_SIZE):
        ...
        if len(Image1_All) == 0:
            raise(RuntimeError("Found 0 images in: " + SourceImage1_Path + "\n"
                               "Supported image extensions are: " +
                               ",".join(IMG_EXTENSIONS)))

        self.Image1_All = Image1_All
        self.Image2_All = Image2_All # 图像的路径list
        self.transform = test_transform
        self.loader = loader # 图像打开形式  PIL.Image
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            ...
            # Y Y Cr Cb ir_path vis_path 对应分解为亮度分量和色彩分量；色彩分量使用的是vis的；
...
class GET_TrainDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return y1, y2, cr, cb, path1,path2 # HR 和 LR path 内容；
