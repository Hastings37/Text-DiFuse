...
def main():
    ...
    while step < args.max_iterations:
        ...
        cond, cond_hs, cond1, cond1_hs = common_to_latent(ae, vis, vis_gt, type_name, mean_var_dict, 'vis', args.device)
        ...
        if (step + 1) % args.val_interval == 0:
            ...
            target_val_images = 50  # <--- 修改2：设定目标验证图片数量
            ...
            with torch.no_grad():
                for val_idx, val_data in enumerate(val_loader):
                    ...
                    cond_val, cond_hs_val, _, _ = common_to_latent(
                        ae, val_vis, val_vis_gt, val_type_name, mean_var_dict, 'vis', args.device
                    )

                    sample_shape = cond_val.shape
                    model_kwargs_val = {'condition': cond_val}
                    ...
                    restored_latent = latent_to_common(
                        pred_norm_latent, val_type_name, mean_var_dict, 'vis', args.device
                    )

                    pred_img = ae.decode(restored_latent, cond_hs_val)
