...
def main():
    ...
    parser.add_argument('--Task_type', type=str,
                        default='VIS-IR')  # Modify parameters to achieve different fusion tasks: VIS-IR, MRI-CT, MRI-PET, MRI-SPECT#
    ...
    for epoch in range(args.epoch):
        ...
        for i, input in enumerate(train_loader):
            cond = {'condition': input[0].to(args.device)} # vis
            cond1 = {'condition': input[1].to(args.device)} # ir
            filename = os.path.basename(str(input[4]))[:-3]
            """GET GT"""
            output_GT1 = diffusion.p_sample_loop(# vis_gt
                diffusion_stage1,
                diffusion_stage2,
            ...
            output_GT2 = diffusion.p_sample_loop(# ir_gt
                diffusion_stage1,
                diffusion_stage2,
                Fusion_Control_Model,
                input[0].shape,
