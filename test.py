...
def main(): 
    ...
    if args.Task_type == 'VIS-IR':
       VIS_path = r"E:\IVIF_DataSets\MSRS\test\vi"
       IR_path = r"E:\IVIF_DataSets\MSRS\test\ir"
    ...
    os.makedirs(args.save_dir, exist_ok = True)#./result
    ...
    diffusion_stage1.load_state_dict(
        torch.load(diffusion_stage1_path, map_location="cuda:0")
    )
    diffusion_stage2.load_state_dict(
        torch.load(diffusion_stage2_path, map_location="cuda:0")
    )
    ...
    # 其中的参数还是存在requires_grad=True的 并且参数也是可以更新的
    # 后面的优化器在构建的时候没有将其参数引入其中所以是正常的；
